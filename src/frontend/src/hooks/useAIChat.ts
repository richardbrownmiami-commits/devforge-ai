...
const SYSTEM_PROMPT =
  "You are an expert web app builder.\n" +
  "RULES (MUST FOLLOW STRICTLY):\n" +
  "1. Always return a COMPLETE, SINGLE <!DOCTYPE html> file in ONE code block (lang: html).\n" +
  "2. Include all CSS in <style> tags and all JavaScript in <script> tags -- single file only.\n" +
  "3. The app must be fully functional, visually polished with dark theme and working interactions.\n" +
  "4. After the code block, write a SHORT explanation (2-3 sentences max).\n" +
  "5. NEVER return partial code, snippets, or placeholder comments like '// rest of code here'.\n" +
  "6. NEVER return code outside the html code block.\n" +
  "7. When fixing errors, return the COMPLETE corrected HTML file.";
...
async function tryOpenRouter(
  ...
  if (!res.ok) {
    ...
    if (res.status === 401)
      throw new Error(
        "Invalid OpenRouter API key. Go to Settings \u2192 API Keys.",
      );
...
async function geminiWithFallback(
  ...
  const candidates = [
    preferred,
    ...GEMINI_FALLBACKS.filter((m) => m !== preferred),
  ];
  for (const model of candidates) {
    if (signal.aborted) break;
...
async function tryGroqModel(
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    if (res.status === 401)
      throw new Error("Invalid Groq API key. Go to Settings \u2192 API Keys.");
    if (res.status === 429) throw new Error("RATE_LIMITED");
...
async function groqWithFallback(
  ...
  const candidates = [
    preferred,
    ...GROQ_FALLBACKS.filter((m) => m !== preferred),
  ];
  for (const model of candidates) {
    if (signal.aborted) break;
    onModel(`Groq (${model})`);
...
async function tryGitHubModel(
  ...
  if (!res.ok) {
    ...
    if (res.status === 401)
      throw new Error(
        "Invalid GitHub Models token. Go to Settings \u2192 API Keys.",
      );
    if (res.status === 429) throw new Error("RATE_LIMITED");
      ...
      if (local.length > 0) {
        ...
        loadChatMessagesAsync(projectName).then(async (idbMsgs) => {
          if (idbMsgs.length > 0) {
            ...
            const ghMsgs = await pullSessionFromGitHub(projectName).catch(
              () => null,
            );
  ...
  const sendMessage = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        if (provider === "groq") {
          if (!groqKey)
            throw new Error("No Groq API key. Go to Settings \u2192 API Keys.");
          ...
          if (!githubModelsKey)
            throw new Error(
              "No GitHub Models token. Go to Settings \u2192 API Keys.",
            );
          reply = await tryGitHubModel(
            githubModelsKey,
            githubModelsModel,
            history,
            signal,

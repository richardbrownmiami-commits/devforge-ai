...
return new Response(JSON.stringify({error:"not found"}),{status:404,headers:{"content-type":"application/json"}});}async function handleCron() {const{PROVIDERS}=await import("config.ts");for(const p of PROVIDERS){const keys=await getKeys(p.name);for(const ke of keys){const h=await getHealth(p.name,ke.id);if(h.status==="dead"||h.status==="expired")continue;try{const hdrs:any={"Content-Type":"application/json"};if(p.type==="openai")hdrs["Authorization"]="Bearer "+ke.apiKey;const testBody={model:p.models[0],messages:[{role:"user",content:"ping"}],max_tokens:1};const resp=await fetch(p.baseUrl+"/chat/completions",{method:"POST",headers:hdrs,body:JSON.stringify(testBody)});if(resp.ok){h.status="active";h.lastCheck=Date.now();h.consecutiveFailDays=0;h.lastError="";}else{h.failCount++;if(resp.status===401||resp.status===403){h.consecutiveFailDays++;h.lastError=resp.status+": auth failed";}else{h.lastError=resp.status+": upstream error";}if(h.consecutiveFailDays>=EVICT_DAYS){h.status="expired";await logEviction(p.name,ke.id,"5_day_eviction: "+h.lastError);}}await setHealth(p.name,ke.id,h);}catch(e:any){h.failCount++;h.lastError="network: "+e.message;h.consecutiveFailDays++;if(h.consecutiveFailDays>=EVICT_DAYS){h.status="expired";await logEviction(p.name,ke.id,"5_day_eviction: "+h.lastError);}await setHealth(p.name,ke.id,h);}}}}export default {
